...
REX_EXTERN(__imp__rex_sub_8241D620);
REX_EXTERN(__imp__rex_sub_8241D230);
REX_EXTERN(__imp__rex_sub_8241CD88);
REX_EXTERN(__imp__rex_sub_8241D220);
REX_EXTERN(__imp__rex_sub_8217A470);
REX_EXTERN(__imp__rex_sub_8241BE78);
REX_EXTERN(__imp__rex_sub_8217B430);
REX_EXTERN(__imp__rex_sub_8241C308);
REX_EXTERN(__imp__rex_sub_8217B7B0);
REX_EXTERN(__imp__rex_sub_82419E98);
REX_EXTERN(__imp__rex_sub_82420BA8);

// --- draws -----------------------------------------------------------------

// D3DDevice_DrawIndexedVertices(dev, primType, baseVertexIndex, startIndex, indexCount)
extern "C" REX_FUNC(rex_sub_8241D620) {
  ...
  __imp__rex_sub_8241D620(ctx, base);
...
extern "C" REX_FUNC(rex_sub_8241D230) {
  const uint32_t dev = ctx.r3.u32;
  const uint32_t prim = ctx.r4.u32;
  ...
  const mcla::native_gfx::ConstantDirtyMasks dirty =
      mcla::native_gfx::Active() ? mcla::native_gfx::ReadConstantDirtyMasks(base, dev)
                                 : mcla::native_gfx::ConstantDirtyMasks{};
  __imp__rex_sub_8241D230(ctx, base);
  if (mcla::gfx_probe::Enabled()) {
...
extern "C" REX_FUNC(rex_sub_8241CD88) {
  const uint32_t dev = ctx.r3.u32;
  const uint32_t prim = ctx.r4.u32;
  const uint32_t count = ctx.r5.u32;
  const uint32_t stride = ctx.r6.u32;
  __imp__rex_sub_8241CD88(ctx, base);
...
extern "C" REX_FUNC(rex_sub_8241D220) {
  const uint32_t dev = ctx.r3.u32;
  __imp__rex_sub_8241D220(ctx, base);
  if (mcla::native_gfx::Active()) {
...
extern "C" REX_FUNC(rex_sub_8241C308) {
  const uint32_t dev = ctx.r3.u32;
  ...
  __imp__rex_sub_8241C308(ctx, base);
...
extern "C" REX_FUNC(rex_sub_8217B7B0) {
  const bool probe_was_enabled = mcla::gfx_probe::Enabled();
  __imp__rex_sub_8217B7B0(ctx, base);
...
// D3DDevice_Swap. Pass-through unless the native runtime has taken over
// presentation (mcla_native_gfx_present): then the native frame is presented
// and the guest swap is suppressed (single producer for the guest output).
//
...
extern "C" REX_FUNC(rex_sub_82419E98) {
  ...
  __imp__rex_sub_82419E98(ctx, base);
  // Continuous mode presents AFTER the guest swap, not before: the Xenia
  // command processor's IssueSwap paints the shared guest output here, so
  // presenting earlier (at EndFrame) let Xenia overwrite the native frame.
  // Painting last makes the native frame the one the vsync worker shows, while
  // the guest swap still ran so the CP closed its submission (no TDR).
  if (mcla::native_gfx::Active()) {
    mcla::native_gfx::PresentContinuousAtSwap();
  }
}
...
extern "C" REX_FUNC(rex_sub_82420BA8) {
  const uint32_t dev = ctx.r3.u32;
  const uint32_t flags = ctx.r4.u32;
  ...
  __imp__rex_sub_82420BA8(ctx, base);
  if (mcla::native_gfx::Active()) {
